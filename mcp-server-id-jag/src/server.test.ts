...
test("exchanges an ID-JAG for an access token and exposes whoami", async () => {
  ...
  try {
    ...
    const mintedBody = (await mintResponse.json()) as { assertion: string };
    ...
    const rpcBody = parseSseJsonResponse(
      await mcpResponse.text(),
    ) as {
      result: { content: Array<{ text: string }> };
    };
    const whoami = JSON.parse(rpcBody.result.content[0]!.text) as {
...
function parseSseJsonResponse(body: string): unknown {
  const dataLine = body
    .split("\n")
