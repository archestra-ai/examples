...
export async function createApp(config = {}) {
    ...
    app.post("/token", async (req, res) => {
        ...
        try {
            ...
            res.status(400).json({
                error: "invalid_grant",
                error_description: error instanceof Error ? error.message : "The supplied assertion was rejected",
            });
        }
    });
    app.post("/mcp", async (req, res) => {
        ...
        if (!accessToken) {
            res
                .status(401)
                .setHeader("WWW-Authenticate", `Bearer resource_metadata="${resolvedConfig.baseUrl}/.well-known/oauth-protected-resource"`)
                .json({
                error: "invalid_token",
...
function registerTools(server, accessToken) {
...
function authenticateClient(req, configuredClient) {
    ...
    if (clientId !== configuredClient.clientId ||
